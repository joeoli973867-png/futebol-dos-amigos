      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Gestão inteligente para sua pelada
          </p>
        </div>
...
export default function PeladaPro() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "players" | "games" | "finance" | "group"
  >("dashboard");
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  // State with Supabase persistence
  const [group, setGroup] = useState<GroupInfo & { inviteCode?: string }>(
    INITIAL_GROUP,
  );
  ...
  const [supabaseStatus, setSupabaseStatus] = useState<
    "connected" | "error" | "placeholder"
  >("placeholder");

  // Modal States
  const [modalType, setModalType] = useState<
    "player" | "attendance" | "transaction" | "group" | null
  >(null);
  ...
  const mensalistas = players.filter(
    (p) => p.isMonthly === true && p.status === "active",
  );
  const avulsos = players.filter(
    (p) => p.isMonthly === false && p.status === "active",
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch Attendances
        const { data: attendancesData, error: attendancesError } =
          await supabase
            .from("attendances")
            .select("*")
            .eq("group_id", memberData.group_id)
  ...
  const topPlayers = useMemo(() => {
    ...
    return stats.sort((a, b) => b.appearances - a.appearances).slice(0, 3);
  }, [players, attendances]);
  ...
  const paidPlayerIds = useMemo(() => {
    ...
    return new Set(
      transactions
        .filter((t) => {
          if (t.category !== "monthly_fee" || !t.playerId || !t.date)
            return false;
          try {
            const tDate =
  ...
  const monthlyStats = useMemo(() => {
    const activePlayers = players.filter(
      (p) => p.status === "active" && p.isMonthly,
    );
    const paidCount = activePlayers.filter((p) =>
      paidPlayerIds.has(String(p.id)),
    ).length;
  ...
  const monthlyFinanceStats = useMemo(() => {
    ...
    const expectativaMensalistas = mensalistas.reduce(
      (acc, p) => acc + (p.monthlyFee || 0),
      0,
    );
  ...
  const createGroup = async (name: string) => {
    ...
    try {
      ...
      if (groupError) {
        ...
        throw new Error(
          `Erro ao criar grupo: ${groupError.message} (${groupError.details || "sem detalhes"})`,
        );
      }
      ...
      if (groupData) {
        ...
        if (memberError) {
          ...
          throw new Error(
            `Erro ao entrar no grupo como admin: ${memberError.message}`,
          );
        }
        ...
        throw new Error(
          "Grupo criado, mas não foi possível recuperar os dados. Tente atualizar a página.",
        );
      }
    } catch (error: any) {
  ...
  const updatePlayer = async (id: string, updates: Partial<Player>) => {
    ...
    if (updates.isMonthly !== undefined)
      supabaseUpdates.is_monthly = updates.isMonthly;
    if (updates.monthlyFee !== undefined)
      supabaseUpdates.monthly_fee = updates.monthlyFee;

    await supabase.from("players").update(supabaseUpdates).eq("id", id);
  };
  ...
  const updateAttendance = async (id: string, updates: Partial<Attendance>) => {
    if (userRole !== "admin") return;
    setAttendances(
      attendances.map((a) => (a.id === id ? { ...a, ...updates } : a)),
    );

    const supabaseUpdates: any = {};
    if (updates.date !== undefined) supabaseUpdates.date = updates.date;
    if (updates.playerIds !== undefined)
      supabaseUpdates.player_ids = updates.playerIds;
    if (updates.absentIds !== undefined)
      supabaseUpdates.absent_ids = updates.absentIds;

    await supabase.from("attendances").update(supabaseUpdates).eq("id", id);
  };
  ...
  const updateTransaction = async (
    ...
    if (updates.playerId !== undefined)
      supabaseUpdates.player_id = updates.playerId;
    else if ("playerId" in updates) supabaseUpdates.player_id = null;

    await supabase.from("transactions").update(supabaseUpdates).eq("id", id);
  };
        ...
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col items-end">
              ...
              <span
                className={`text-sm font-bold ${cashBalance >= 0 ? "text-green-600" : "text-red-600"}`}
              >
                R${" "}
          ...
          <AnimatePresence mode="wait">
                      ...
                      {userRole === "admin" && (
                        <button
                          onClick={() => {
                            ...
                            const gameTime =
                              group.time.split(",")[1]?.trim() || group.time;
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">
                        Jogadores mais presentes nas partidas
                      </p>
                    </div>
                  </div>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {mensalistas.map((player) => (
                      <div
                        key={player.id}
                        className="bg-white dark:bg-[#161B22] p-5 rounded-3xl border border-slate-200 dark:border-[#30363D] shadow-sm flex justify-between items-center group"
                      >
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          {!paidPlayerIds.has(String(player.id)) && (
                            ...
                            >
                              <span className="text-xs font-bold uppercase px-1">
                                Cobrar
                              </span>
                            </button>
                          )}
                ...
                <section className="mt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {avulsos.map((player) => (
                      <div
                        key={player.id}
                        className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-center opacity-90"
                      >
                        <div className="flex items-center gap-4">
            ...
            {activeTab === "games" && (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                className="space-y-6 bg-white dark:bg-transparent p-6 rounded-3xl"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    {/* Ajustado: Cor escura no modo claro e branca no modo dark */}
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
                      Jogos & Presença
                    </h3>
                    {/* Ajustado: Cinza médio para melhor leitura em ambos os fundos */}
                    <p className="text-slate-500 dark:text-slate-400">
                      Controle quem compareceu a cada partida
                    </p>
                  </div>
                  ...
                  {userRole === "admin" && (
                    <button
                      ...
                      /* Ajustado: Botão azul/escuro no claro e mantendo o estilo no dark, ou variando */
                      className="bg-slate-900 dark:bg-blue-600 text-white px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
                    >
                      <Plus size={20} />
                      Novo Jogo
                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {attendances.map((att) => {
                    ...
                    return (
                      <div
                        key={att.id}
                        className="bg-white rounded-3xl p-6 shadow-sm border border-gray-50"
                      >
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2 text-gray-500">
                            <Calendar size={16} />
                            <span className="text-sm font-bold">
                              {format(parseISO(att.date), "dd 'de' MMMM", {
                                locale: ptBR,
                              })}
                            </span>
                          </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">
                      Controle de entradas, saídas e mensalidades
                    </p>
                  </div>
                  {userRole === "admin" && (
                    <div className="flex gap-2">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-3xl shadow-sm border border-gray-50 overflow-hidden">
                      <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                        ...
                        <button className="text-sm text-gray-400 font-bold hover:text-black">
                          Ver Tudo
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-bold text-green-400">
                            + R${" "}
                            {monthlyFinanceStats.income.toLocaleString(
                              "pt-BR",
                              { minimumFractionDigits: 2 },
                            )}
                          </span>
                        ...
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-bold text-red-400">
                            - R${" "}
                            {monthlyFinanceStats.expense.toLocaleString(
                              "pt-BR",
                              { minimumFractionDigits: 2 },
                            )}
                          </span>
                        ...
                        <div className="pt-4 border-t border-gray-50">
                          <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(monthlyStats.paid / (monthlyStats.total || 1)) * 100}%`,
                              }}
                            />
                          </div>
                          <p className="text-[10px] text-gray-400 mt-2 text-right font-bold uppercase tracking-wider">
                            {Math.round(
                              (monthlyStats.paid / (monthlyStats.total || 1)) *
                                100,
                            )}
                            % Arrecadado
                          </p>
            ...
            {activeTab === "group" && (
              ...
              >
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">
                    Ajuste as informações básicas da sua pelada
                  </p>
                </div>
                <div className="bg-white dark:bg-[#161B22] rounded-3xl p-8 shadow-sm border border-gray-50 dark:border-[#30363D] space-y-8">
                  {userRole === "admin" ? (
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          value={group.name}
                          onChange={(e) =>
                            updateGroupSettings({ name: e.target.value })
                          }
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent dark:bg-slate-900 text-slate-900 dark:text-white focus:border-blue-500 outline-none transition-colors font-bold"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateGroupSettings({ location: e.target.value })
                          }
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-900 text-slate-900 dark:text-white focus:border-blue-500 outline-none transition-colors font-bold"
                        />
                        {group.location && (
                          <div className="flex items-center gap-4 mt-2">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              /* Ajustado: Cor que se adapta e hover azul */
                              className="text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex items-center gap-1"
                            >
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              /* Ajustado: Mesmo padrão para manter a consistência */
                              className="text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex items-center gap-1"
                            >
                              <MapPin size={12} /> Waze
                            </a>
                      ...
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          value={group.time}
                          onChange={(e) =>
                            updateGroupSettings({ time: e.target.value })
                          }
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-900 text-slate-900 dark:text-white focus:border-blue-500 outline-none transition-colors font-bold"
                        />
                      </div>
                    ...
                    <div className="space-y-6">
                      <div className="p-4 bg-yellow-50 border border-yellow-100 rounded-2xl text-yellow-700 text-sm">
                        Apenas administradores podem alterar as configurações do
                        grupo.
                      </div>
                    ...
                    <div className="bg-black text-white rounded-3xl p-8 relative overflow-hidden">
                      <div className="relative z-10">
                        ...
                        <p className="text-xs opacity-60 mt-2">
                          Compartilhe este código para que outros jogadores
                          possam entrar no grupo.
                        </p>
                      </div>
...
function AttendanceForm({
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ date, playerIds: presentIds, absentIds });
      }}
      className="space-y-6"
    >
      <div className="space-y-2">
